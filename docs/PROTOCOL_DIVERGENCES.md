...
| Payload encoding (`payload_b64`) | Must be base64url (no padding) and decode to canonical `EnvelopeV1` bytes (`CLIENT_RELAY_PROTOCOL_V1.md:15`, `CLIENT_RELAY_PROTOCOL_V1.md:16`, `CLIENT_RELAY_PROTOCOL_V1.md:70`; canonical bytes in `protocol.md:16`). | Relay now validates inbound payloads by tolerant decode (base64url/base64, padded/unpadded) on client `send` and federation `relay_forward`; inbound payload strings are normalized by trimming surrounding whitespace before persistence/forwarding. Client-facing outbound re-encodes per connection preference inferred from observed client `send`, defaulting to legacy padded base64 for receive-only/unknown clients. Corrupt queued payloads detected at delivery/pull/forward time are removed to avoid infinite retry/drop loops (`internal/model/payload_b64.go`, `internal/api/ws.go`, `internal/federation/peering.go`, `internal/storeforward/client.go`). | **Partially aligned (compat mode)** |
| `send_ok` timestamp names/shape | `send_ok` uses `msg_id` and optional `received_at`/`expires_at` pair (`CLIENT_RELAY_PROTOCOL_V1.md:128`, `CLIENT_RELAY_PROTOCOL_V1.md:142`, `CLIENT_RELAY_PROTOCOL_V1.md:144`). | Relay emits `send_ok` only after durable persist and includes canonical `msg_id`, paired `received_at`/`expires_at` (Unix seconds), while retaining legacy `at` as an alias of `received_at` for compatibility. `received_at` currently maps to relay acceptance time from stored message creation timestamp (`internal/api/ws.go`, `internal/model/message.go`, `internal/storeforward/client.go`). | **Partially aligned (compat alias retained)** |
...
| `ack` per-device binding | Ack binding must be `(wayfarer_id, device_id)`; one device ack must not suppress another (`CLIENT_RELAY_PROTOCOL_V1.md:275`, `CLIENT_RELAY_PROTOCOL_V1.md:281`). | Client `ack` now binds directly to current connection identity (`delivery_id`/`wayfarer_id+device_id`) instead of transient per-message tracking; when `device_id` is absent, fallback remains wayfarer-only for legacy clients. Queue indexing is still wayfarer-scoped, with per-device suppression enforced in store-forward filtering (`internal/api/ws.go`, `internal/storeforward/client.go`, `internal/store/bbolt_store.go`). | **Partially aligned (compat fallback retained)** |
...
- Relay normalizes inbound `payload_b64` by trimming surrounding whitespace before it is persisted/forwarded.
...
- Corrupt queued payloads are deleted on first decode failure during pull/delivery/federation forwarding so they do not cause repeated retries/log spam.
...
| `-ack-driven-suppression` flag | New CLI gate controls suppression semantics: default `false` preserves legacy mark-on-push behavior; when `true`, suppression is driven by durable client ack state (`ack_state`) **or existing legacy `delivery_state`** as transitional compatibility to avoid duplicate redelivery when toggling modes on existing stores. Push no longer writes suppression state in this mode (`cmd/relay/main.go`, `internal/api/ws.go`, `internal/storeforward/engine.go`, `internal/storeforward/client.go`, `internal/store/bbolt_store.go`). Startup logs include a migration warning when enabled. | **VERIFY (migration control)** |
