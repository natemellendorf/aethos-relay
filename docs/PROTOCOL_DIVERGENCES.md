...
| `-ack-driven-suppression` flag | New CLI gate controls suppression semantics: default `false` preserves legacy mark-on-push behavior; when `true`, suppression is driven by durable client ack state (`ack_state`) and push no longer marks suppression state (`cmd/relay/main.go`, `internal/api/ws.go`, `internal/storeforward/engine.go`, `internal/store/bbolt_store.go`). | **VERIFY (migration control)** |
