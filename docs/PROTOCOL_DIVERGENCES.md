...
1. Client wire shape still diverges on strict canonical identity requirements (`device_id` remains optional for backward compatibility), timestamps (`at` vs `received_at`), and error schema (`msg_id` string instead of `code`/`message`).
