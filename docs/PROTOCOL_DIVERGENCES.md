...
| `messages` object fields | Each pulled message should expose `msg_id`, `from`, `payload_b64`, `received_at` (`CLIENT_RELAY_PROTOCOL_V1.md:170`, `CLIENT_RELAY_PROTOCOL_V1.md:185`). | Pull response now uses a dedicated wire entry with Unix-second integers (`received_at` + legacy `at`) instead of marshaling `model.Message` RFC3339 timestamps. Cutover note: pull `at` changed from RFC3339 string to epoch-seconds int for protocol alignment (`internal/api/ws.go`, `internal/model/message.go`). | **Partially aligned (cutover + compat alias)** |
