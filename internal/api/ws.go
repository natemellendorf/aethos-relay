...
func NewWSHandler(store store.Store, clients *model.ClientRegistry, maxTTL time.Duration, allowedOrigins string, devMode bool, relayID string) *WSHandler {
	...
	return &WSHandler{
		...
		drainLimits:       gossipv1.DefaultDrainSessionLimits(),
		lbMode:            "sticky",
	}
}
...
func (h *WSHandler) handleSummary(session *clientSession, event gossipv1.Event) error {
	...
	if len(want) == 0 {
		...
		metrics.IncrementGossipDrainNoProgressRound()
		drain.Stop(gossipv1.DrainStopReasonNoEligibleItems)
		return stopSession(gossipv1.DrainStopReasonNoEligibleItems, "summary produced no wanted items")
	}
...
func (h *WSHandler) buildRecipientSummary(queueRecipient string, deliveryIdentity string, afterCursor string) (gossipv1.SummaryPayload, string, error) {
	...
	if deliveryIdentity != "" {
		filteredPreview := make([]string, 0, len(previewIDs))
		for _, id := range previewIDs {
			...
			filteredPreview = append(filteredPreview, id)
		}
		previewIDs = filteredPreview
