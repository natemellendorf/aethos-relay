...
func newClientForWSHandler() *model.Client {
	client := &model.Client{Conn: &wsTestConn{}, Send: make(chan []byte, 16)}
...
func assertErrorFrameStrict(t *testing.T, frame model.WSFrame, wantCode model.ErrorCode, wantMessage string) {
	t.Helper()
	if frame.Type != model.FrameTypeError {
		t.Fatalf("expected error frame, got %q", frame.Type)
	}
	...
	if frame.MsgID != "" {
		t.Fatalf("strict errors must not include msg_id alias, got %q", frame.MsgID)
	}
}

func TestHandleHelloAllowsOptionalDeviceID(t *testing.T) {
	h, _ := newWSHandlerWithSpyStore(t)
	h.SetStrictClientRelayV1(false)
...
func TestHandleHelloStrictRequiresDeviceID(t *testing.T) {
	h, _ := newWSHandlerWithSpyStore(t)
	h.SetStrictClientRelayV1(true)

	client := newClientForWSHandler()
	h.handleHello(client, &model.WSFrame{Type: model.FrameTypeHello, WayfarerID: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"})

	resp := readQueuedFrame(t, client)
	assertErrorFrameStrict(t, resp, model.ErrorCodeInvalidPayload, "device_id required")
}

func TestHandleHelloStrictRejectsInvalidWayfarerFormat(t *testing.T) {
	h, _ := newWSHandlerWithSpyStore(t)
	h.SetStrictClientRelayV1(true)

	client := newClientForWSHandler()
	h.handleHello(client, &model.WSFrame{Type: model.FrameTypeHello, WayfarerID: "Wayfarer-A", DeviceID: "device-a"})

	resp := readQueuedFrame(t, client)
	assertErrorFrameStrict(t, resp, model.ErrorCodeInvalidWayfarerID, "wayfarer_id must be 64 lowercase hex")
}

func TestHandleFrameStrictRequiresHelloFirst(t *testing.T) {
	h, _ := newWSHandlerWithSpyStore(t)
	h.SetStrictClientRelayV1(true)

	client := newClientForWSHandler()
	keepOpen := h.handleFrame(client, &model.WSFrame{Type: model.FrameTypePull})
	if keepOpen {
		t.Fatal("strict mode should close connection for pre-hello non-hello frame")
	}

	resp := readQueuedFrame(t, client)
	assertErrorFrameStrict(t, resp, model.ErrorCodeInvalidPayload, "hello required before other frames")
}
...
func TestLegacyModeDeliveryMetricsPushOnlyAndAckNoIncrementAfterPush(t *testing.T) {
	h, st := newWSHandlerWithSpyStore(t)
	h.SetStrictClientRelayV1(false)
	h.SetAckDrivenSuppression(false)
...
func TestHandleSendStrictSendOKCanonicalFieldsOnly(t *testing.T) {
	h, st := newWSHandlerWithSpyStore(t)
	h.SetStrictClientRelayV1(true)

	sender := newClientForWSHandler()
	sender.WayfarerID = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"

	payload := "AQEBAAAAIAABAgMEBQYHCAkKCwwNDg8QERITFBUWFxgZGhscHR4fAgAAABNtYW5pZmVzdC1maXhlZC0wMDAxAwAAAAxoZWxsby1hZXRob3M"
	to := "000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f"

	h.handleSend(sender, &model.WSFrame{Type: model.FrameTypeSend, To: to, PayloadB64: payload, TTLSeconds: 120})

	frame := decodeFrameMap(t, readQueuedPayload(t, sender))
	...
	if len(st.persisted) != 1 || st.persisted[0].Payload != payload {
		t.Fatalf("expected persisted canonical payload, got %#v", st.persisted)
	}
}

func TestHandleSendStrictRejectsNonCanonicalPayloadB64(t *testing.T) {
	h, _ := newWSHandlerWithSpyStore(t)
	h.SetStrictClientRelayV1(true)

	client := newClientForWSHandler()
	client.WayfarerID = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
	to := "000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f"

	for _, payload := range []string{"QQ==", " \n\tQQ\r "} {
		h.handleSend(client, &model.WSFrame{Type: model.FrameTypeSend, To: to, PayloadB64: payload})
		frame := readQueuedFrame(t, client)
		assertErrorFrameStrict(t, frame, model.ErrorCodeInvalidPayload, "invalid payload_b64")
	}
}

func TestHandleSendStrictRejectsToMismatch(t *testing.T) {
	h, _ := newWSHandlerWithSpyStore(t)
	h.SetStrictClientRelayV1(true)
	...
	h.handleSend(client, &model.WSFrame{
		Type:       model.FrameTypeSend,
	...
	frame := readQueuedFrame(t, client)
	assertErrorFrameStrict(t, frame, model.ErrorCodeToMismatch, "send.to does not match EnvelopeV1 recipient")
}

func TestHandleSendStrictDefaultsTTLToCanonical3600(t *testing.T) {
	h, st := newWSHandlerWithSpyStore(t)
	h.SetStrictClientRelayV1(true)
	...
	gotTTL := st.persisted[0].ExpiresAt.Sub(st.persisted[0].CreatedAt)
	if gotTTL < 3599*time.Second || gotTTL > 3601*time.Second {
		t.Fatalf("strict ttl default mismatch: got %v", gotTTL)
	}
}
...
func TestHandleFrameErrorResponsesIncludeCanonicalAndLegacyFields(t *testing.T) {
	tests := []struct {
		...
		{name: "unknown frame type", frame: model.WSFrame{Type: "mystery"}, wantCode: model.ErrorCodeInvalidPayload, wantMessage: "unknown frame type"},
		{name: "relay frame blocked on client connection", frame: model.WSFrame{Type: model.FrameTypeRelayHello}, wantCode: model.ErrorCodeInvalidPayload, wantMessage: "relay frame type not allowed on client connections"},
		{name: "hello missing wayfarer_id", frame: model.WSFrame{Type: model.FrameTypeHello}, wantCode: model.ErrorCodeInvalidWayfarerID, wantMessage: "wayfarer_id required"},
		{name: "send before hello", frame: model.WSFrame{Type: model.FrameTypeSend, To: "wayfarer-b", PayloadB64: "QQ=="}, wantCode: model.ErrorCodeAuthFailed, wantMessage: "not authenticated"},
		{
		...
		{name: "pull before hello", frame: model.WSFrame{Type: model.FrameTypePull}, wantCode: model.ErrorCodeAuthFailed, wantMessage: "not authenticated"},
	}
...
func TestHandleFrameInternalFailuresMapToInternalError(t *testing.T) {
	tests := []struct {
		...
		{name: "persist failure", configure: func(st *wsStoreSpy) { st.persistErr = errors.New("persist failed") }, frame: model.WSFrame{Type: model.FrameTypeSend, To: "wayfarer-b", PayloadB64: "QQ=="}, wantMessage: "failed to persist message"},
		{name: "ack failure", configure: func(st *wsStoreSpy) { st.markDeliveredErr = errors.New("ack failed") }, frame: model.WSFrame{Type: model.FrameTypeAck, MsgID: "msg-1"}, wantMessage: "failed to acknowledge message"},
		{name: "pull failure", configure: func(st *wsStoreSpy) { st.getQueuedErr = errors.New("pull failed") }, frame: model.WSFrame{Type: model.FrameTypePull}, wantMessage: "failed to pull messages"},
	}
...
func TestHandleSendInfersPayloadEncodingPreference(t *testing.T) {
	t.Skip("client model currently canonicalizes outbound payload encoding preference")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			h.handleSend(sender, &model.WSFrame{Type: model.FrameTypeSend, To: "wayfarer-b", PayloadB64: tt.payload, TTLSeconds: 120})
...
func TestHandleSendNormalizesPayloadWhitespaceBeforePersist(t *testing.T) {
	...
	h.handleSend(sender, &model.WSFrame{Type: model.FrameTypeSend, To: "wayfarer-b", PayloadB64: " \n\tQQ==\r ", TTLSeconds: 120})
...
func TestDeliverToRecipientIncludesCanonicalAndLegacyMessageTimestamps(t *testing.T) {
	...
	h.deliverToRecipient(&model.Message{ID: "msg-1", From: "wayfarer-a", To: "wayfarer-b", Payload: "QQ==", CreatedAt: createdAt, ExpiresAt: createdAt.Add(time.Hour)})
...
func TestDeliverToRecipientStrictDoesNotEmitLegacyAtField(t *testing.T) {
	h, _ := newWSHandlerWithSpyStore(t)
	h.SetStrictClientRelayV1(true)

	recipient := newClientForWSHandler()
	recipient.WayfarerID = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
	recipient.DeviceID = "device-a"
	recipient.DeliveryID = storeforward.DeliveryIdentity(recipient.WayfarerID, recipient.DeviceID)
	h.clients.Register(recipient)
	...
	h.deliverToRecipient(&model.Message{
		...
		To:        recipient.WayfarerID,
		Payload:   "AQEBAAAAIAABAgMEBQYHCAkKCwwNDg8QERITFBUWFxgZGhscHR4fAgAAABNtYW5pZmVzdC1maXhlZC0wMDAxAwAAAAxoZWxsby1hZXRob3M",
		CreatedAt: time.Now().UTC(),
		ExpiresAt: time.Now().UTC().Add(time.Hour),
	})

	frame := decodeFrameMap(t, readQueuedPayload(t, recipient))
	if _, ok := frame["at"]; ok {
		t.Fatalf("strict message push must not include legacy at: %#v", frame)
	}
	if _, ok := frame["received_at"]; !ok {
		t.Fatalf("strict message push must include received_at: %#v", frame)
	}
}
...
func TestDeliverToRecipientEncodesPayloadByRecipientPreference(t *testing.T) {
	...
	h.deliverToRecipient(&model.Message{ID: "msg-url-pref", From: "wayfarer-a", To: "wayfarer-b", Payload: "+/8=", CreatedAt: createdAt, ExpiresAt: createdAt.Add(time.Hour)})
...
func TestHandlePullEncodesPayloadByRecipientPreference(t *testing.T) {
	...
	st.queued[client.WayfarerID] = []*model.Message{{ID: "msg-pull-url-pref", From: "wayfarer-a", To: client.WayfarerID, Payload: "+/8=", CreatedAt: createdAt, ExpiresAt: createdAt.Add(time.Hour)}}
	...
	if frame.Messages[0].PayloadB64 != "-_8" {
		t.Fatalf("expected base64url payload in pull response, got %q", frame.Messages[0].PayloadB64)
	}
	decoded, err := model.DecodePayloadB64(frame.Messages[0].PayloadB64)
	if err != nil {
...
func TestHandlePullStrictDefaultsLimitTo50(t *testing.T) {
	h, st := newWSHandlerWithSpyStore(t)
	h.SetStrictClientRelayV1(true)

	client := newClientForWSHandler()
	client.WayfarerID = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
	client.DeviceID = "device-a"
	client.DeliveryID = storeforward.DeliveryIdentity(client.WayfarerID, client.DeviceID)

	for i := 0; i < 60; i++ {
		st.queued[client.WayfarerID] = append(st.queued[client.WayfarerID], &model.Message{
			ID:        "msg-" + time.Unix(int64(i), 0).UTC().Format("150405"),
			From:      "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
			To:        client.WayfarerID,
			Payload:   "AQEBAAAAIAABAgMEBQYHCAkKCwwNDg8QERITFBUWFxgZGhscHR4fAgAAABNtYW5pZmVzdC1maXhlZC0wMDAxAwAAAAxoZWxsby1hZXRob3M",
			CreatedAt: time.Now().UTC(),
	...
	h.handlePull(client, &model.WSFrame{Type: model.FrameTypePull})

	frame := readQueuedFrame(t, client)
	if frame.Type != model.FrameTypeMessages {
		t.Fatalf("expected messages frame, got %q", frame.Type)
	}
	if len(frame.Messages) != 50 {
		t.Fatalf("strict default pull limit mismatch: got %d want %d", len(frame.Messages), 50)
	}
}

func TestHandlePullStrictEmitsCanonicalMessageEntry(t *testing.T) {
	h, st := newWSHandlerWithSpyStore(t)
	h.SetStrictClientRelayV1(true)
	...
	if _, ok := entry["to"]; ok {
		t.Fatalf("unexpected to field in strict pull: %#v", entry)
	}
}
...
func TestHandlePullRemovesCorruptPayloadFromQueue(t *testing.T) {
	...
	st.queued[client.WayfarerID] = []*model.Message{{ID: "msg-corrupt-pull", From: "wayfarer-a", To: client.WayfarerID, Payload: "%%%", CreatedAt: createdAt, ExpiresAt: createdAt.Add(time.Hour)}}
...
func TestHandlePullStrictDropsExpiredMessage(t *testing.T) {
	h, st := newWSHandlerWithSpyStore(t)
	h.SetStrictClientRelayV1(true)
	...
	h.handlePull(client, &model.WSFrame{Type: model.FrameTypePull, Limit: 10})
	resp := readQueuedFrame(t, client)
	if resp.Type != model.FrameTypeMessages || len(resp.Messages) != 0 {
		t.Fatalf("expected empty pull for strict expired messages, got %+v", resp)
	}

	st.mu.Lock()
	defer st.mu.Unlock()
	if len(st.removedMsgIDs) != 1 || st.removedMsgIDs[0] != "msg-expired" {
		t.Fatalf("expected expired message removal, got %v", st.removedMsgIDs)
	}
}
...
func TestDeliverToRecipientRemovesCorruptPayloadFromQueue(t *testing.T) {
	...
	msg := &model.Message{ID: "msg-corrupt-deliver", From: "wayfarer-a", To: "wayfarer-b", Payload: "%%%", CreatedAt: createdAt, ExpiresAt: createdAt.Add(time.Hour)}
...
func TestDeliverToRecipientDefaultsToLegacyBase64Padding(t *testing.T) {
	...
	h.deliverToRecipient(&model.Message{ID: "msg-std-pref", From: "wayfarer-a", To: "wayfarer-b", Payload: "-_8", CreatedAt: createdAt, ExpiresAt: createdAt.Add(time.Hour)})
...
func TestHandlePullMessagesKeepLegacyAndAddCanonicalTimestamps(t *testing.T) {
	...
	st.queued[client.WayfarerID] = []*model.Message{{ID: "msg-pull-1", From: "wayfarer-a", To: client.WayfarerID, Payload: "QQ==", CreatedAt: createdAt, ExpiresAt: createdAt.Add(time.Hour)}}
	...
	if _, ok := entry["expires_at"]; !ok {
		t.Fatalf("expected legacy expires_at field, got %#v", entry["expires_at"])
	}
	if _, ok := entry["to"]; !ok {
		t.Fatalf("expected legacy to field, got %#v", entry)
	}
}
