...
func main() {
	...
	gossipDrainRoundBudget := flag.Int("gossip-drain-round-budget", 0, "Per-session round budget for gossip drain rounds (0 disables budget)")
	gossipDrainByteBudget := flag.Int64("gossip-drain-byte-budget", 0, "Per-session byte budget (sent+received) for gossip drain sessions (0 disables budget)")
	gossipDrainWallClockBudget := flag.Duration("gossip-drain-wallclock-budget", 0, "Per-session wall-clock budget for gossip drain sessions (0 disables budget)")
	gossipDrainNoProgressCap := flag.Int("gossip-drain-no-progress-cap", 0, "Consecutive no-progress rounds before stopping gossip drain session (0 disables cap)")
	gossipDrainYieldEveryRounds := flag.Int("gossip-drain-yield-every-rounds", 0, "Yield long gossip sessions every N rounds for fairness (0 disables yield)")
